...
export default function CreateGame() {
	const { connect, updateQueryParams, queryParams, sendJsonMessage, isConnected, lastJsonMessage } = useSocket()!;
	...
	function handleLastMessage() {
		...
		if (lastJsonMessage && lastJsonMessage['type'] === 'error') {
			window.alert(`${lastJsonMessage['params']['message']}, Probably start this from first again`);
		...
		if (lastJsonMessage && lastJsonMessage['type'] === 'info') {
			if (lastJsonMessage['params']['players']) {
				setPlayers(() => lastJsonMessage['params']['players'].map((p: string) => {
					return { name: p, position: 0, inJail: false, color: randomColor() }
				}));
			}
		...
		<div>
			...
			<ul className="list-disc">
				{players.map((player, index) => (
					<li key={index} className="text-slate-400">{player.name}</li>
				))
				}</ul>
